...
var IMG_INVITE = 'https://raw.githubusercontent.com/google/material-design-icons/master/content/2x_web/ic_add_black_36dp.png';
...
function createInfoWindow(place, infoWindow, $scope, Points) {
  ...
  btn.addEventListener('click', function () {
    ...
    if (btn.dataset.working == 0) {
      infoWindow.close();
      var points = parseInt(Points.get()) + parseInt(place.points);
      Points.set(points);
  ...
  btn2.setAttribute('class', 'btn-work');
  btn2.innerHTML = '<img src="' + IMG_INVITE + '">';
  iwframe.appendChild(btn2);
...
.controller('MapCtrl', function($scope, $state, $ionicPopup, Tasks, Location, Points) {

  $scope.showConfirm = function() {
     var confirmPopup = $ionicPopup.confirm({
       title: 'Invite people nearby',
       template: '<ion-checkbox ng-model="isChecked1">Liu QI</ion-checkbox>'
                +'<ion-checkbox ng-model="isChecked2">Li Yingjie</ion-checkbox>'
                +'<ion-checkbox ng-model="isChecked3">Vanchi</ion-checkbox>'
     });
  ...
  navigator.geolocation.getCurrentPosition(function (pos) {
    ...
    google.maps.event.addDomListener(arBtn, 'click', function () {
      window.location = "ar-master/examples/geo_ar/index.html?points="+parseInt(Points.get());
    });
